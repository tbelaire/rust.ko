...
impl<K, V> BoxedNode<K, V> {
    ...
    fn from_internal(node: Box<InternalNode<K, V>>) -> Self {
        unsafe {
            BoxedNode {
                ptr: Unique::new(Box::into_raw(node) as *mut LeafNode<K, V>),
            }
        }
    }
...
unsafe impl<BorrowType, K: Sync, V: Sync, Type> Sync for NodeRef<BorrowType,
                                                                 K,
                                                                 V,
                                                                 Type> {
}
...
impl<BorrowType, K, V, Type> NodeRef<BorrowType, K, V, Type> {
    ...
    pub fn ascend(self)
                  -> Result<Handle<NodeRef<BorrowType, K, V, marker::Internal>,
                                   marker::Edge>,
                            Self> {
        if self.as_leaf().parent.is_null() {
            ...
            Ok(Handle {
                node: NodeRef {
                    ...
                    node: unsafe {
                        NonZero::new(self.as_leaf().parent as *mut LeafNode<K,
                                                                            V>)
                    },
                    root: self.root,
                    _marker: PhantomData,
...
impl<K, V> NodeRef<marker::Owned, K, V, marker::Leaf> {
    ...
    > {
        let ptr =
            self.as_leaf() as *const LeafNode<K, V> as *const u8 as *mut u8;
...
impl<'a, K, V> NodeRef<marker::Mut<'a>, K, V, marker::Internal> {
    ...
    pub fn push_front(&mut self, key: K, val: V, edge: Root<K, V>) {
        ...
        unsafe {
            slice_insert(self.keys_mut(), 0, key);
            slice_insert(self.vals_mut(), 0, val);
            slice_insert(slice::from_raw_parts_mut(self.as_internal_mut()
                                                       .edges
                                                       .as_mut_ptr(),
                                                   self.len() + 1),
                         0,
        ...
        unsafe {
            ...
            let edge = match self.reborrow_mut().force() {
                ...
                ForceResult::Internal(internal) => {
                    let edge = ptr::read(internal.as_internal()
                                                 .edges
                                                 .get_unchecked(idx + 1));
        ...
        unsafe {
            ...
            let edge = match self.reborrow_mut().force() {
                ...
                ForceResult::Internal(mut internal) => {
                    ...
                    for i in 0..old_len {
                        Handle::new_edge(internal.reborrow_mut(), i)
                            .correct_parent_link();
                    }
...
impl<BorrowType, K, V, NodeType> Handle<NodeRef<BorrowType, K, V, NodeType>,
                                        marker::KV> {
    pub fn new_kv(node: NodeRef<BorrowType, K, V, NodeType>,
                  idx: usize)
                  -> Self {
...
impl<BorrowType, K, V, NodeType, HandleType> Handle<NodeRef<BorrowType,
                                                            K,
                                                            V,
                                                            NodeType>,
                                                    HandleType> {
...
impl<'a, K, V, NodeType, HandleType> Handle<NodeRef<marker::Mut<'a>,
                                                    K,
                                                    V,
                                                    NodeType>,
                                            HandleType> {
...
impl<BorrowType, K, V, NodeType> Handle<NodeRef<BorrowType, K, V, NodeType>,
                                        marker::Edge> {
    pub fn new_edge(node: NodeRef<BorrowType, K, V, NodeType>,
                    idx: usize)
                    -> Self {
        // Necessary for correctness, but in a private module
        debug_assert!(idx <= node.len());
...
impl<'a, K, V> Handle<NodeRef<marker::Mut<'a>, K, V, marker::Leaf>,
                      marker::Edge> {
    ...
    pub fn insert(mut self,
        ...
        if self.node.len() < CAPACITY {
            ...
            let ptr = if self.idx <= B {
                unsafe {
                    Handle::new_edge(left.reborrow_mut(), self.idx)
                        .insert_fit(key, val)
                }
            } else {
                unsafe {
                    Handle::new_edge(right.as_mut()
                                          .cast_unchecked::<marker::Leaf>(),
                                     self.idx - (B + 1))
                        .insert_fit(key, val)
                }
            };
...
impl<'a, K, V> Handle<NodeRef<marker::Mut<'a>, K, V, marker::Internal>,
                      marker::Edge> {
    ...
    fn insert_fit(&mut self, key: K, val: V, edge: Root<K, V>) {
        ...
        unsafe {
            ...
            slice_insert(slice::from_raw_parts_mut(self.node
                                                       .as_internal_mut()
                                                       .edges
                                                       .as_mut_ptr(),
                                                   self.node.len()),
            ...
            for i in (self.idx + 1)..(self.node.len() + 1) {
                Handle::new_edge(self.node.reborrow_mut(), i)
                    .correct_parent_link();
            }
        }
    }
...
impl<BorrowType, K, V> Handle<NodeRef<BorrowType, K, V, marker::Internal>,
                              marker::Edge> {
    pub fn descend(self) -> NodeRef<BorrowType, K, V, marker::LeafOrInternal> {
        NodeRef {
            height: self.node.height - 1,
            node: unsafe {
                self.node.as_internal().edges.get_unchecked(self.idx).as_ptr()
            },
            root: self.node.root,
            _marker: PhantomData,
        }
...
impl<'a, K: 'a, V: 'a, NodeType> Handle<NodeRef<marker::Immut<'a>,
                                                K,
                                                V,
                                                NodeType>,
                                        marker::KV> {
...
impl<'a, K: 'a, V: 'a, NodeType> Handle<NodeRef<marker::Mut<'a>,
                                                K,
                                                V,
                                                NodeType>,
                                        marker::KV> {
...
impl<'a, K, V, NodeType> Handle<NodeRef<marker::Mut<'a>, K, V, NodeType>,
                                marker::KV> {
    pub fn kv_mut(&mut self) -> (&mut K, &mut V) {
        unsafe {
            let (mut keys, mut vals) = self.node
                                           .reborrow_mut()
                                           .into_slices_mut();
            (keys.get_unchecked_mut(self.idx), vals.get_unchecked_mut(self.idx))
        }
...
impl<'a, K, V> Handle<NodeRef<marker::Mut<'a>, K, V, marker::Leaf>, marker::KV> {
    ...
    pub fn remove(mut self)
                  -> (Handle<NodeRef<marker::Mut<'a>, K, V, marker::Leaf>,
                             marker::Edge>,
                      K,
                      V) {
        unsafe {
    ...
    pub fn split
        ...
        unsafe {
            ...
            for i in 0..(new_len + 1) {
                Handle::new_edge(new_root.as_mut().cast_unchecked(), i)
                    .correct_parent_link();
            }
    ...
    pub fn merge(mut self)
        ...
        unsafe {
            ...
            if self.node.height > 1 {
                ptr::copy_nonoverlapping(right_node.cast_unchecked()
                                                   .as_internal()
                                                   .edges
                                                   .as_ptr(),
                                         left_node.cast_unchecked()
                                                  ...
                                                  .edges
                                                  .as_mut_ptr()
                                                  .offset(left_len as isize +
                                                          1),
                                         right_len + 1);

                for i in left_len + 1..left_len + right_len + 2 {
                    Handle::new_edge(left_node.cast_unchecked().reborrow_mut(),
                                     i)
                        .correct_parent_link();
                }

                heap::deallocate(*right_node.node as *mut u8,
                                 mem::size_of::<InternalNode<K, V>>(),
                                 mem::align_of::<InternalNode<K, V>>());
            } else {
                heap::deallocate(*right_node.node as *mut u8,
...
impl<BorrowType, K, V, HandleType> Handle<NodeRef<BorrowType,
                                                  K,
                                                  V,
