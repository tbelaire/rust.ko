        ...
        fn clone_subtree<K: Clone, V: Clone>(
            ...
            match node.force() {
                Leaf(leaf) => {
                    ...
                    {
                        let mut out_node = match out_tree.root
                                                         .as_mut()
                                                         .force() {
                            Leaf(leaf) => leaf,
                ...
                Internal(internal) => {
                    let mut out_tree = clone_subtree(internal.first_edge()
                                                             .descend());
          ...
          Q: Ord
{
    ...
    fn replace(&mut self, key: K) -> Option<K> {
        match search::search_tree::<marker::Mut, K, (), K>(self.root.as_mut(),
                                                           &key) {
...
pub struct OccupiedEntry<'a, K: 'a, V: 'a> {
    handle: Handle<NodeRef<marker::Mut<'a>, K, V, marker::LeafOrInternal>,
                   marker::KV>,
                ...
                match search::search_tree(self.root.as_ref(), key) {
                    Found(kv_handle) => {
                        match kv_handle.left_edge().force() {
                            Leaf(bottom) => bottom,
                            Internal(internal) => {
                                last_leaf_edge(internal.descend())
                            }
                        }
                    }
                ...
                match search::search_tree(self.root.as_ref(), key) {
                    Found(kv_handle) => {
                        match kv_handle.right_edge().force() {
                            Leaf(bottom) => bottom,
                            Internal(internal) => {
                                first_leaf_edge(internal.descend())
                            }
                        }
                    }
                    GoDown(bottom) => bottom,
    ...
    fn next(&mut self) -> Option<(K, V)> {
        ...
        loop {
            match cur_handle.right_kv() {
                ...
                Err(last_edge) => unsafe {
                    cur_handle =
                        unwrap_unchecked(last_edge.into_node()
        ...
        *self.length -= 1;
        ...
        let (small_leaf, old_key, old_val) = match self.handle.force() {
            ...
            Internal(mut internal) => {
                ...
                let to_remove = first_leaf_edge(internal.right_edge()
                                                        .descend())
                                    .right_kv()
                                    .ok();
        ...
        unsafe {
            ...
            if is_left {
                match handle.reborrow_mut().right_edge().descend().force() {
                    ...
                    Internal(mut internal) => {
                        internal.push_front(k, v, edge.unwrap())
                    }
                }
            } else {
