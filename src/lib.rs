...
#[no_mangle]
pub unsafe fn rust_Bulk_transport(srb: *mut Struct_scsi_cmnd, us: *mut Struct_us_data) -> c_int {
    ...
    println!("Bulk Command S 0x{:x} T 0x{:x} L {} F {} Trg {} LUN {} CL {}",
             my_le32_to_cpu((*bcb).Signature),
             (*bcb).Tag,
             my_le32_to_cpu((*bcb).DataTransferLength),
             (*bcb).Flags,
             ((*bcb).Lun >> 4),
             ((*bcb).Lun & 0x0F),
             (*bcb).Length);
    ...
    if transfer_length != 0 {
        ...
        if result == CONST_USB_STOR_XFER_SHORT &&
           (*srb).sc_data_direction as usize == Enum_dma_data_direction::DMA_FROM_DEVICE as usize &&
           transfer_length as i32 - my_scsi_get_resid(srb) == CONST_US_BULK_CS_WRAP_LEN as i32 {

            let mut sg: *mut Struct_scatterlist = null_mut();
            let mut offset: c_uint = 0;

            if (usb_stor_access_xfer_buf(bcs as *mut u8,
                                         CONST_US_BULK_CS_WRAP_LEN,
                                         srb,
                                         &mut sg,
                                         &mut offset,
                                         Enum_xfer_buf_dir::FROM_XFER_BUF) ==
                CONST_US_BULK_CS_WRAP_LEN &&
    ...
    println!("Bulk Status S 0x{:x} T 0x{:x} R {} Stat 0x{:x}",
             ...
             residue,
             (*bcs).Status);
    if !((*bcs).Tag == (*us).tag || ((*us).fflags & CONST_US_FL_BULK_IGNORE_TAG != 0)) ||
    ...
    if residue != 0 && ((*us).fflags & CONST_US_FL_IGNORE_RESIDUE != 0) {
        ...
        // commands.
        //
        if (*bcs).Status == CONST_US_BULK_STAT_OK as u8 && my_scsi_get_resid(srb) == 0 &&
           (((*(*srb).cmnd) == CONST_INQUIRY && transfer_length == 36) ||
            ((*(*srb).cmnd) == CONST_READ_CAPACITY && transfer_length == 8)) {

            (*us).fflags |= CONST_US_FL_IGNORE_RESIDUE;

        } else {
            residue = core::cmp::min(residue, transfer_length);
            my_scsi_set_resid(srb,
                              core::cmp::max(my_scsi_get_resid(srb), residue as c_int));
        }
    }
    ...
    return if status == CONST_US_BULK_STAT_OK {
        ...
        if (fake_sense != 0) {
            memcpy((*srb).sense_buffer as *mut _ as *mut _,
